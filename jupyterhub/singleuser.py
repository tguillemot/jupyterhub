...
from kernel_gateway.gatewayapp import (
    KernelGatewayApp,
    aliases as gateway_aliases,
)
...
# Thierry: This allow to control the hub
aliases = dict(notebook_aliases)
aliases.update({
    'user': 'SingleUserNotebookApp.user',
    'group': 'SingleUserNotebookApp.group',
    'cookie-name': 'HubAuth.cookie_name',
    'hub-prefix': 'SingleUserNotebookApp1.hub_prefix',
    'hub-host': 'SingleUserNotebookApp1.hub_host',
    'hub-api-url': 'SingleUserNotebookApp1.hub_api_url',
    'base-url': 'SingleUserNotebookApp1.base_url',
})
flags = dict(notebook_flags)
flags.update({
    'disable-user-config': ({
        'SingleUserNotebookApp1': {
...
class SingleUserNotebookApp1(NotebookApp):
    examples = ""
    subcommands = {}
    version = __version__
    classes = NotebookApp.classes + [HubAuth]
    ...
    def _confirm_exit(self):
        # disable the exit confirmation for background notebook processes
        # Thierry : notebook ask confirmation for ^C. It's just a way to confirm
        # stop
        ioloop.IOLoop.instance().stop()
    ...
    def migrate_config(self):
        if self.disable_user_config:
            ...
            super(SingleUserNotebookApp1, self).migrate_config()

    @property
    def config_file_paths(self):
        path = super(SingleUserNotebookApp1, self).config_file_paths
    ...
    @property
    def nbextensions_path(self):
        path = super(SingleUserNotebookApp1, self).nbextensions_path
    ...
    def start(self):
        # Thierry : Can be removed as is just asking super
        super(SingleUserNotebookApp1, self).start()
    ...
    def init_webapp(self):
        ...
        self.hub_auth.login_url = self.hub_host + self.hub_prefix
        s['csp_report_uri'] = self.hub_host + url_path_join(self.hub_prefix, 'security/csp-report')
        super(SingleUserNotebookApp1, self).init_webapp()
        self.patch_templates()
...
# Try to define the kernel gateway server

aliases_kernel = dict(gateway_aliases)
aliases_kernel.update({
    'user': 'SingleUserKernelGatewayApp.user',
    'group': 'SingleUserKernelGatewayApp.group',
    'cookie-name': 'HubAuth.cookie_name',
    'hub-prefix': 'SingleUserKernelGatewayApp.hub_prefix',
    'hub-host': 'SingleUserKernelGatewayApp.hub_host',
    'hub-api-url': 'SingleUserKernelGatewayApp.hub_api_url',
    'base-url': 'SingleUserKernelGatewayApp.base_url',
})


class SingleUserKernelGatewayApp(KernelGatewayApp):
    examples = ""
    subcommands = {}
    version = __version__
    classes = KernelGatewayApp.classes + [HubAuth]
    ...
    aliases = aliases_kernel


def main(argv=None):
    return SingleUserKernelGatewayApp.launch_instance(argv)
